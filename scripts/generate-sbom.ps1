...
& $tool "$repoRoot\AjazzBattery.sln" --output $artifacts --filename 'AjazzBatteryMonitor-v1.3.0.sbom.json' --output-format Json --runtime win-x64 --configuration Release --exclude-test-projects --disable-package-restore --set-name 'AJAZZ Battery Monitor' --set-version '1.3.0'
